...
public class Application implements CommandLineRunner {
	...
	public Utente trovaUtente(int parseInt) throws Exception {
		Utente result = ur.findById(parseInt);
		if(result != null) {
			return result;
		} else {
			throw new Exception();
	...
	public LocalDate impostaData(String s, Utente utente) throws Exception {
		try {
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
			LocalDate data = LocalDate.parse(s, formatter);
			LocalDate dataOra = LocalDate.now();
			List<Booking> lista = br.findByUser(utente.getUsername());
